...
export class RedshiftDataService {
    ...
    private async upsert<TValue>(_: { database: string, workgroupName: string }, value: TValue, tableQualifiers: { schema: string, table: string }) {
        const { table, schema } = tableQualifiers;
        // TODO: dynamically build out the select/insert clauses dynamically
        //         try {
        //             const upsertQuery = `
        // WITH ${table}_x AS (SELECT '${value.user_id}' as user_id, '${value.profile_id}' as profile_id, '${value.email}' as email, '${value.date}' as date, '${value.weight}' as weight, '${value.source}' as source, '${value.created_at}' as created_at)
        // UPDATE ${schema}.${table}
        // SET
        //   patient = a.patient,
        //   data = a.data
        // FROM ${table}_x AS a
        // WHERE ${schema}.${table}.id = a.id;
        ...
        //   a.data
        // FROM ${table}_x AS a
        // LEFT JOIN ${schema}.${table} AS b
        //   ON a.id = b.id
        // WHERE b.id IS NULL;`;

        //             return await this.query(_, upsertQuery);
        //         } catch (error: any) {
        //             console.error({ error, detail: `"${schema}.${table}" upsert failed` });
        //         }
    }
}