...
describe(dataSourceConfigFactory.name + ' (postgres)', () => {
  const baseOptions = {
    database: 'mydb',
    host: 'localhost',
    port: '5432',
  ...
  beforeEach(() => {
    infoSpy = jest.spyOn(console, 'info').mockImplementation(() => { /* silence */ });
  });
  ...
  it('produces a postgres config with sensible defaults', () => {
  ...
  it('passes schema through', () => {
    const config = dataSourceConfigFactory('x', { ...baseOptions, schema: 'private' }, []);
    expect(config.schema).toBe('private');
  });
  ...
  it('honors an explicit logging=true flag', () => {
    const config = dataSourceConfigFactory('x', { ...baseOptions, logging: true }, []);
